...
class TxtDatasetProcessing(Dataset):
    def __init__(self, data_path, txt_path, img_path, txt_filename, label_filename, sen_len, corpus):
    ...
    def __getitem__(self, index):
        ...
        img_filename = os.path.join(self.img_path, self.txt_filename[index].strip('.txt')+'.npy')

        fp = open(filename, 'r')
#        print filename
        txt = torch.LongTensor(np.zeros(self.sen_len, dtype=np.int64))
        img = torch.from_numpy(np.load(img_filename).squeeze()).float()
